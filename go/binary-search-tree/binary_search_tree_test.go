...
var reverseTestCases = []SortedDataTestCase{
	{
		description: "can reverse single number",
		input:       []int{2},
		expected:    []int{2},
	},
	{
		description: "can reverse if second number is smaller than first",
		input:       []int{1, 2},
		expected:    []int{2, 1},
	},
	{
		description: "can reverse if second number is same as first",
		input:       []int{2, 2},
		expected:    []int{2, 2},
	},
	{
		description: "can reverse if second number is greater than first",
		input:       []int{2, 3},
		expected:    []int{3, 2},
	},
	{
		description: "can reverse complex tree",
		input:       []int{1, 2, 3, 5, 6, 7},
		expected:    []int{7, 6, 5, 3, 2, 1},
	},
}
...
func TestReverse(t *testing.T) {
	for _, td := range reverseTestCases {
		t.Run(td.description, func(t *testing.T) {
			tree := makeBst(td.input)
			if tree == nil {
				t.Fatalf("bst should not be nil")
