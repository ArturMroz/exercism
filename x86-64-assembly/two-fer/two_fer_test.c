...
void test_yet_another_name_given(void) {
    /* TEST_IGNORE(); */
    char buffer[BUFFER_SIZE];

    two_fer("Alexander the Great", buffer);
    TEST_ASSERT_EQUAL_STRING("One for Alexander the Great, one for me.", buffer);
}
...
int main(void) {
    ...
    RUN_TEST(test_yet_another_name_given);
    return UNITY_END();
}
